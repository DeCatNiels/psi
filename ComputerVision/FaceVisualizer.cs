...
namespace ComputerVision
{
    class FaceVisualizer
    {
        ...
        // Constructor
        public FaceVisualizer(Pipeline pipeline)
        {
            // create detectedObjects receiver
            this.DetectedFacesIn = pipeline.CreateReceiver<ImageAnalysis>(this, ReceiveDetectedFaces, nameof(this.DetectedFacesIn));
        ...
        public Receiver<ImageAnalysis> DetectedFacesIn { get; private set; }
        ...
        private void ReceiveDetectedFaces(ImageAnalysis input, Envelope envelope)
        {
            ...
            foreach (FaceDescription detectedFace in input.Faces)
            {
                try
                {
                    ...
                    {
                        Rectangle rect = new Rectangle(detectedFace.FaceRectangle.Left, detectedFace.FaceRectangle.Top, detectedFace.FaceRectangle.Width, detectedFace.FaceRectangle.Height);
                        Point textPoint = new Point(detectedFace.FaceRectangle.Left + 10, detectedFace.FaceRectangle.Top + 10);
                        lastReceivedImageInput.Resource.DrawRectangle(rect, Color.Red, 3);
                        lastReceivedImageInput.Resource.DrawText(detectedFace.Age + "\n" + detectedFace.Gender, textPoint, new SolidBrush(Color.Red));
                    }
                }
                catch (Exception ex)
                {
